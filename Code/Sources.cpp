...
void Avion::run() {
    while (this->actif) {
        ...
        if (this->carburant <= 0 && this->etat != "au sol") {
            this->etat = "crash";
            this->enDeplacement = false;
            std::cout << "Avion " << this->id << " a crashé par manque de carburant" << std::endl;
        }
...
CentreControle::CentreControle(Monde* monde) {
...
size_t CentreControle::genererDestinationAleatoire(size_t aeroportDepart) {
    size_t destination;
    do {
...
void CentreControle::demarrerTousLesAvions() {
